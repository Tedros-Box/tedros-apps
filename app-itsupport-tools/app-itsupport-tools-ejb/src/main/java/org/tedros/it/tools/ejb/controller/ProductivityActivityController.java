...
public class ProductivityActivityController extends TSecureEjbController<ProductivityActivityDTO>
	...
	public TResult<List<ProductivityActivityDTO>> findUserIdAndDateRange(TAccessToken token, Long userId,
	...
	@Override
	public TResult<List<ActivitySummaryDTO>> getWindowUsageSummary(TAccessToken token, Long userId, LocalDateTime start,
			LocalDateTime end) {
		try {
			List<ActivitySummaryDTO> summary = repository.getWindowUsageSummary(userId, start, end);
			return new TResult<>(TState.SUCCESS, summary);
		} catch (Exception e) {
			return super.processException(token, null, e);
		}
	}
