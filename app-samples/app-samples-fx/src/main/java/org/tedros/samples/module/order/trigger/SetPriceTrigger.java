...
public class SetPriceTrigger extends TTrigger<Product> {
	...
	public void run(TEvent event, Product value, Product old) {
		...
		TDynaPresenter<OrderItemMV> detailPresenter =  (TDynaPresenter<OrderItemMV>) super.getForm().gettPresenter();
		// Get the behavior from the given presenter, in this case TDetailFieldBehavior because the 
		// OrderItemMV are annotated with TDetailTableViewPresenter which use it.
		TDetailFieldBehavior<OrderItemMV, OrderItem> detailBehavior =  
				(TDetailFieldBehavior<OrderItemMV, OrderItem>) detailPresenter.getBehavior();
		// From this behavior we can get the main presenter
		// As the OrderModule use the TGroupView the returned presenter 
		// will be a TGroupPresenter.
		ITPresenter pre = detailBehavior.getModulePresenter();
		TDynaPresenter<OrderMV> masterPresenter = (TDynaPresenter) 
				((TGroupPresenter)pre).getSelectedView().gettPresenter();
		// With the main presenter we can get the OrderMV
		OrderMV mv = masterPresenter.getModelView();
		// Getting the LegalPerson and CostCenter from OrderMV
