/**
 * 
 */
package org.tedros.stock;

/**
 * The language resource key domain
 * 
 * Run the org.tedros.fx.util.LanguageKeysBuilder to create the constants from
 * your language resource. Type the resource name without the locale, for
 * example to read the App_en.properties just type App
 * 
 * @author Davis Dun
 *
 */
public interface STCKKey {

	static final String APP_STOCK = "#{app.stock}";
	static final String MENU_STOCK = "#{menu.stock}";
	static final String MODULE_DESC_PRODUCTS = "#{module.desc.products}";
	static final String MODULE_INVENTORY = "#{module.inventory}";
	static final String MODULE_INVENTORY_DESC = "#{module.inventory.desc}";
	static final String MODULE_PRODUCTS = "#{module.products}";
	static final String REACH_MINIMUM_AMOUNT = "#{reach.minimum.amount}";
	static final String STCK_INFO_ENTRYTYPE = "#{stck.info.entrytype}";
	static final String STCK_INFO_OUTTYPE = "#{stck.info.outtype}";
	static final String TITLE_REPORT_INVENTORY = "#{title.report.inventory}";
	static final String TITLE_REPORT_PRODUCT = "#{title.report.product}";
	static final String TOOLTIP_SIZE = "#{tooltip.size}";
	static final String VIEW_ENTRY_TYPE = "#{view.entry.type}";
	static final String VIEW_ENTRY_TYPE_DESC = "#{view.entry.type.desc}";
	static final String VIEW_INVENTORY = "#{view.inventory}";
	static final String VIEW_INVENTORY_DESC = "#{view.inventory.desc}";
	static final String VIEW_INVENTORY_REPORT = "#{view.inventory.report}";
	static final String VIEW_INVENTORY_REPORT_DESC = "#{view.inventory.report.desc}";
	static final String VIEW_OUT_TYPE = "#{view.out.type}";
	static final String VIEW_OUT_TYPE_DESC = "#{view.out.type.desc}";
	static final String VIEW_PRODUCT = "#{view.product}";
	static final String VIEW_PRODUCT_DESC = "#{view.product.desc}";
	static final String VIEW_PRODUCT_REPORT = "#{view.product.report}";
	static final String VIEW_PRODUCT_REPORT_DESC = "#{view.product.report.desc}";
	static final String VIEW_STOCK_CONFIG = "#{view.stock.config}";
	static final String VIEW_STOCK_CONFIG_DESC = "#{view.stock.config.desc}";
	static final String VIEW_STOCK_ENTRY = "#{view.stock.entry}";
	static final String VIEW_STOCK_ENTRY_DESC = "#{view.stock.entry.desc}";
	static final String VIEW_STOCK_OUT = "#{view.stock.out}";
	static final String VIEW_STOCK_OUT_DESC = "#{view.stock.out.desc}";
	static final String WITH_INSUFFICIENT_STOCK = "#{with.insufficient.stock}";
	
}
